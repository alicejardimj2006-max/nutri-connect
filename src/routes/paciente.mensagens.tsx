...
const msgs = [
  { from: "them", t: "Oi João! Como você está se sentindo com o novo plano?", h: "14:30" },
  { from: "me", t: "Muito bem! Consegui seguir o cardápio.", h: "14:31" },
  { from: "them", t: "Que ótimo! Como foi a semana?", h: "14:32" },
];
  ...
  return (
    <div className="grid h-[calc(100vh-10rem)] gap-4 overflow-hidden rounded-2xl border bg-card shadow-card md:grid-cols-[280px_1fr]">
      <aside className="border-r">
        <div className="border-b p-4"><h2 className="font-semibold">Conversas</h2></div>
        <ul>
          {conversas.map((c) => (
            <li key={c.id}>
      ...
      <section className="flex min-w-0 flex-col">
        <div className="border-b p-4"><div className="font-semibold">{conversas.find((c) => c.id === active)?.nome}</div><div className="text-xs text-muted-foreground">online</div></div>
