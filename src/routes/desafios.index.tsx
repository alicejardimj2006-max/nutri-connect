...
import { getEarnedBadges } from "@/lib/community";
      ...
      <div className="rounded-3xl border border-border bg-card p-6 sm:p-8 mb-10 shadow-xs">
        <div className="flex items-center justify-between mb-5">
          ...
          <span className="text-xs text-muted-foreground font-medium">
            {joinedCount > 0
              ...
              : "Comece um desafio para colecionar distintivos"}
          </span>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {earnedBadges.map((badge) => (
            ...
            >
              <span className="text-3xl">{badge.achieved ? badge.icon : "🔒"}</span>
              <div>
                <p className="text-sm font-bold text-foreground">{badge.label}</p>
                <p className="text-[11px] text-muted-foreground">
                  {badge.count === 1
                    ? "Conclua 1 desafio"
                    : `Conclua ${badge.count} desafios`}
                </p>
              </div>
            </div>
          ))}
