  ...
  const renderDialogue = (character?: Character, text?: string) => {
    ...
    return (
      <div className="flex flex-col sm:flex-row items-end sm:items-start gap-4 mb-8 animate-in slide-in-from-bottom-4 duration-500">
        <div className="text-7xl sm:text-8xl shrink-0 animate-bounce pt-2">
          {character.avatar}
        </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-background/95 backdrop-blur-sm sm:p-4">
      ...
      <div className="flex-1 flex flex-col w-full max-w-3xl mx-auto bg-card sm:rounded-[2rem] sm:border-2 border-border sm:shadow-2xl overflow-hidden relative">
            ...
            <div className="w-full max-w-md mx-auto flex-1 flex flex-col items-center justify-center text-center animate-in zoom-in-95 duration-500">
              {hasPassed ? (
                <>
                  ...
                  <div className="flex gap-4 w-full">
                    <div className="flex-1 bg-secondary/50 rounded-2xl p-4 border border-border">
                      <div className="text-muted-foreground text-sm font-bold uppercase tracking-wider mb-1">Total de XP</div>
                      <div className="text-3xl font-black text-amber-500">{lesson.xpReward}</div>
                    </div>
                    <div className="flex-1 bg-secondary/50 rounded-2xl p-4 border border-border">
                      <div className="text-muted-foreground text-sm font-bold uppercase tracking-wider mb-1">Acertos</div>
                      <div className="text-3xl font-black text-accent">{correctCount}/{totalQuestions}</div>
                    </div>
                  </div>
                </>
              ) : (
                <>
                  ...
                  <p className="text-lg text-foreground/80 mb-8">
                    Para ganhar o XP e avançar na trilha, você precisa de pelo menos <strong>{Math.round(lesson.minPassScore * 100)}%</strong> de acertos. 
                    <br/><br/>
                    Você acertou <strong>{correctCount} de {totalQuestions}</strong>. Não desanime, a repetição é a mãe do aprendizado!
                  </p>
                </>
              )}
            </div>
        ...
        {phase === 'activity' && (
          <div className={`absolute bottom-0 left-0 right-0 p-5 sm:p-6 transition-colors duration-300 border-t-2 ${
            isChecking 
              ? isCorrect 
              ...
              {isChecking && (
                <div className="flex-1 w-full animate-in slide-in-from-left-4">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`text-xl sm:text-2xl font-black ${isCorrect ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                      {isCorrect ? 'Mandou bem!' : 'Ops, não foi dessa vez.'}
                    </span>
                  </div>
                  {/* Explanation text */}
                  {(currentActivity as any).explanation && (
                    <p className={`text-sm sm:text-base font-medium mt-2 leading-relaxed ${isCorrect ? 'text-green-800 dark:text-green-300' : 'text-red-800 dark:text-red-300'}`}>
                      {(currentActivity as any).explanation}
                    </p>
                  )}
                </div>
